...
class GenGenerator {
  ...
  void constint(int width, std::string value) {
    out << "if(parser_fail(NailOutStream_write(str_current," << value << ","
        << width << "))) return -1;";
  }
  void generator(constarray& c) {
    switch (c.value.N_type) {
      ...
      case VALUES: {
        ...
        FOREACH(v, c.value.values) { constint(width, intconstant_value(*v)); }
        break;
      }
    }
  }
  void generator(constparser& p) {
    switch (p.N_type) {
      ...
      case CREPEAT:
        generator(*p.crepeat);
        break;  // Emit just one of the elements
      case CINT: {
        int width = boost::lexical_cast<int>(mk_str(p.cint.parser.unsign));
        constint(width, intconstant_value(p.cint.value));
      } break;
      case CREF:
        out << "gen_" << mk_str(p.cref) << "(str_current);";
        break;
      case CSTRUCT:
        FOREACH(field, p.cstruct) { generator(*field); }
        break;
      case CUNION:
  ...
  void generator(parserinner& p, Expr& val, const Scope& scope) {
    switch (p.N_type) {
      case INTEGER: {
        ...
        out << "if(parser_fail(NailOutStream_write(str_current," << val << ","
            << width << "))) return -1;";
        break;
      }
      case STRUCTURE: {
        ...
        fixup << "{/*Context-rewind*/\n NailOutStreamPos  end_of_struct= "
                 "NailOutStream_getpos(str_current);\n";
          ...
          switch (field->N_type) {
            ...
            case TRANSFORM: {
              ...
              FOREACH(stream, field->transform.left) {
                ...
                out << "if(parser_fail(NailOutStream_init(&str_"
                    << mk_str(*stream) << ",4096))) {return -1;}\n";
              ...
              invocation << "if(parser_fail(" << cfunction
                         << "_generate(tmp_arena";
              declaration << "extern  int " << cfunction
                          << "_generate(NailArena *tmp_arena";
              FOREACH(stream, field->transform.left) {
              ...
              FOREACH(param, field->transform.right) {
                switch (param->N_type) {
                  case PDEPENDENCY: {
                    ...
                    declaration << "," << newscope.dependency_type(name) << "* "
                                << name;
                  } break;
                  case PSTREAM:
                    invocation << ","
                               << newscope.stream_ptr(mk_str(param->pstream));
                    declaration << ",NailOutStream *str_"
                                << mk_str(param->pstream);
                    break;
                }
              }
              declaration << ");";
              if (!option::templates) header << declaration.str();
              invocation << "))){return -1;}";
              FOREACH(stream, field->transform.left) {
                newscope.add_stream_definition(mk_str(*stream));
              }
              invocation << "{ NailOutStreamPos t_rewind_eot "
                            "=NailOutStream_getpos(str_current);\n";
              ...
              FOREACH(param, field->transform.right) {
                ...
                invocation << "NailOutStream_reposition(str_current, rewind_"
                           << name << ");";
                invocation << "NailOutStream_write(str_current,dep_" << name
                           << "," << newscope.dependency_width(name) << ");";
              }
              invocation << " NailOutStream_reposition(str_current, "
                            "t_rewind_eot);\n }";
        ...
        // TODO:  We need to properly interleave APPLY and TRANSFORM when
        // generating! Transform at the end is wrong in some cases (when the
        // applied parser relies on a context from the transform)
        out << fixup.str();
        out << "}";
      } break;
      case APPLY: {
      ...
      case WRAP:
        if (p.wrap.constbefore) {
          FOREACH(c, *p.wrap.constbefore) { generator(*c); }
        }
        generator(p.wrap.parser->pr, val, scope);
        if (p.wrap.constafter) {
          FOREACH(c, *p.wrap.constafter) { generator(*c); }
        }
        break;
      ...
      case SELECTP: {
        ...
        FOREACH(c, p.selectp.options) {
          ...
          out << "case " << enum_tag << ":{\n";
          ValExpr expr(tag, &val);
          out << "*" << scope.dependency_ptr(mk_str(p.selectp.dep)) << "="
              << intconstant_value(c->value) << ";";
          generator(c->parser->pr, expr, scope);
          out << "break;}\n";
        }
        out << "}";
      ...
      case NUNION: {
        ...
        // Parentheses or the like might be NECESSARY for bijection. For now,
        // fail?
      ...
      case ARRAY: {
        ...
        switch (p.array.N_type) {
          case MANYONE:
            generator(p.array.manyone->pr, elem, scope);
            break;
          case MANY:
            generator(p.array.many->pr, elem, scope);
            break;
          case SEPBY:
            out << "if(" << iter << "!= 0){";
            generator(p.array.sepby.separator);
            out << "}";
            generator(p.array.sepby.inner->pr, elem, scope);
            break;
          case SEPBYONE:
            out << "if(" << iter << "!= 0){";
            generator(p.array.sepbyone.separator);
            out << "}";
            generator(p.array.sepbyone.inner->pr, elem, scope);
            break;
        }
        out << "}";
      } break;
      ...
      case NAME: {
        ...
        out << "if(parser_fail(gen_" << mk_str(p.name.name)
            << "(tmp_arena,str_current,&" << val << parameters
            << "))){return -1;}";
        break;
      }
    }
 ...
 public:
  GenGenerator(std::ostream& _out, std::ostream& _hdr)
      : os(_out), hdr(_hdr), num_iters(0) {}
  void generator(grammar* grammar) {
    FOREACH(definition, *grammar) {
      if (definition->N_type == CONSTANTDEF) {
        ...
        header << "int gen_" << (name)
               << "(NailArena *tmp_arena,NailOutStream *out," << name
               << " * val" << params << ");" << std::endl;
        out << "int gen_" << (name)
            << "(NailArena *tmp_arena,NailOutStream *str_current," << name
            << " * val" << params << "){";
