...
#define depend_parse(type)                                                   \
  ...
  int type##_depend_generate(NailArena* foo, type##_t* left,                 \
                             type##_t* right) {                              \
    *left = *right;                                                          \
    return 0;                                                                \
  }

depend_parse(uint16) depend_parse(uint32)

    int size_u32_parse(NailArena* foo, NailStream* fragment,
                       NailStream* current, uint32_t* size) {
  if (current->pos + *size > current->size) return -1;
...
int size_u32_generate(NailArena* tmp, NailStream* fragment, NailStream* current,
                      uint32_t* size) {
  // TODO: Refactor into a stream append library!
  if (NailOutStream_grow(current, 8 * fragment->pos) < 0) return -1;
...
int offset_u32_parse(NailArena* tmp, NailStream* fragment, NailStream* current,
                     uint32_t* offset) {
...
int offset_u32_generate(NailArena* tmp, NailStream* fragment,
                        NailStream* current, uint32_t* offset) {
  if (NailOutStream_grow(current, 8 * fragment->pos) < 0) return -1;
  if (current->bit_offset) return -1;
  memcpy(current->data + current->pos, fragment->data, fragment->pos);
  *offset = current->pos;
...
int zip_compression_parse(NailArena* tmp, NailStream* uncompressed,
                          NailStream* compressed, uint16_t* compression_method,
                          uint32_t* uncompressed_size) {
  switch (*compression_method) {
    case 0:
      if (*uncompressed_size != compressed->size) return -1;
...
int zip_compression_generate(NailArena* tmp, NailStream* uncompressed,
                             NailStream* compressed,
...
int zip_end_of_directory_parse(NailArena* tmp, NailStream* filestream,
                               NailStream* directory, NailStream* entire_file) {
...
int zip_end_of_directory_generate(NailArena* tmp, NailStream* filestream,
                                  NailStream* directory,
                                  NailStream* entire_file) {
...
int crc_32_parse(NailArena* tmp_arena, NailStream* str_uncompressed,
                 uint32_t* crc32_val) {
...
int crc_32_generate(NailArena* tmp_arena, NailStream* str_uncompressed,
                    uint32_t* crc32_val) {
