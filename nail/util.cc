...
bool parameter_type_check(parameterlist* param, parameterdefinitionlist* def,
                          const Scope& scope) {
  if (param == NULL && def == NULL) return true;
  if (param != NULL || def != NULL) return false;
  if (param->count != def->count) return false;
  for (int i = 0; i < param->count; i++) {
    if (param->elem[i].N_type != def->elem[i].N_type) return false;
    switch (param->elem[i].N_type) {
      ...
      case DDEPENDENCY: {
        std::string post;
        if (scope.dependency_type(mk_str(param->elem[i].pdependency)) !=
                typedef_type(*def->elem[i].ddependency.type, "", &post) ||
            post != "")
          return false;
        break;
      }
...
std::string int_type(const intp& intp) {
  ...
  if (length <= 8)
    ...
    throw std::range_error("Integer longer than 64 bits");
  return (boost::format("%s%d_t") % (intp.N_type == UNSIGN ? "uint" : "int") %
          length)
      .str();
}
...
std::string typedef_type(const parser& p, std::string name, std::string* post) {
  ...
  switch (inner.N_type) {
    ...
    case OPTIONAL:
      return (boost::format("%s*") % typedef_type(*inner.optional, name, NULL))
          .str();
    case NAME:
    ...
    case FIXEDARRAY:
      if (post) {
        *post = (boost::format("[%s]%s") %
                 intconstant_value(inner.fixedarray.length) % *post)
                    .str();
        return typedef_type(*inner.fixedarray.inner, name, post);
      } else
        return (boost::format("%s *") %
                typedef_type(*inner.fixedarray.inner, name, NULL))
            .str();
    case NUNION:
      ...
      //  %typedef_type(*inner.fixedarray.inner,name)%
      //  intconstant_value(inner.fixedarray.length)).str();
    case STRUCTURE:
    case ARRAY:
    case CHOICE:
    case SELECTP:
...
std::string parameter_template(const definition& def, Scope& scope) {
  ...
  if (def.parser.parameters) {
    FOREACH(param, *def.parser.parameters) {
      if (DSTREAM != param->N_type) continue;
  ...
  if (def.parser.parameters) {
    FOREACH(param, *def.parser.parameters) {
      switch (param->N_type) {
        case DSTREAM:
          if (option::templates)
            params << ", strt_" << mk_str(param->dstream) << " *str_"
                   << mk_str(param->dstream);
          else
...
std::string parameter_invocation(const parameterlist* parameters,
                                 const Scope& scope) {
