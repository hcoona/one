...
/* XXX: We need to get rid of NailStream. Find some way of statically
   determining which NailStream some transform will create and inline calls to
   that. Then implement a coroutine based parser */
class CDirectParser {
  std::ostream& final;
  std::ostream& hdr;
  ...
  std::string int_expr(const char* stream, unsigned int width) {
    ...
    if (option::templates) {
      if (End == Big) {
        return boost::str(boost::format("%s->read_unsigned_big(%d)") % stream %
                          width);
      } else {
        return boost::str(boost::format("%s->read_unsigned_little(%d)") %
                          stream % width);
      }
    } else {
      ...
      if (End == Big) {
        ...
        return boost::str(
            boost::format("read_unsigned_bits_littleendian(%s,%d)") % stream %
            width);
      }
    }
  }
 ...
 public:
    ...
    switch (c.N_type) {
      ...
      case CREPEAT: {
        std::string retrylabel =
            (boost::format("constmany_%d_repeat") % num_iters++).str();
        std::string faillabel =
            (boost::format("constmany_%d_end") % num_iters++).str();
        out << retrylabel << ":\n";
      ...
      case CREF:
        ...
        break;
      case CSTRUCT:
        FOREACH(field, c.cstruct) { p_const(*field, fail); }
        break;
      case CUNION: {
        out << "{/*CUNION*/\n";
        if (option::templates) {
  ...
  void p_struct(const structparser& s, Expr& lval, const std::string fail,
                Scope& scope) {
  ...
  void parser(const parserinner& p, Expr& lval, const std::string fail,
              Scope& scope) {
    switch (p.N_type) {
      case INTEGER:
      ...
      case WRAP:
        if (p.wrap.constbefore) {
          FOREACH(c, *p.wrap.constbefore) { p_const(*c, fail); }
        }
        parser(p.wrap.parser->pr, lval, fail, scope);
        if (p.wrap.constafter) {
          FOREACH(c, *p.wrap.constafter) { p_const(*c, fail); }
        }
        break;
      case CHOICE: {
        int this_choice = num_iters++;
        if (option::templates) {
          out << "{/*CHOICE*/typename strt_current::pos_t back = "
                 "str_current->getpos();\n";
        } else {
          out << "{/*CHOICE*/NailStreamPos back = "
                 "stream_getpos(str_current);\n";
        }
        ...
        FOREACH(c, p.choice) {
          ...
          std::string fallthrough_memo =
              (boost::format("choice_%u_%s_out") % this_choice % tag).str();
      ...
      case ARRAY: {
        ...
        std::string gotofailsep =
            (boost::format("goto fail_repeat_sep_%d;") % this_many).str();
        ...
        switch (p.array.N_type) {
          ...
          case SEPBY:
            ...
            separator = &p.array.sepby.separator;
            break;
          case SEPBYONE:
        ...
        out << "int32_t " << iexpr << "= 0;\n";
        out << "struct { typeof(" << elem << ") elem; void *prev;} *" << temp
            << " = NULL;\n";
        out << "NailArenaPos back_arena = n_arena_save(arena);";
        out << "typeof(" << temp << ") prev_" << temp << ";";
        if (option::templates)
          out << "typename strt_current::pos_t posrep_" << this_many
              << "= str_current->getpos();";
        else
          out << "NailStreamPos posrep_" << this_many
              << "= stream_getpos(str_current);";
        if (separator != NULL)
        ...
        if (separator != NULL) {
          p_const(*separator, gotofailsep);  // Skip backtracking temp when
                                             // failing the separator
          out << "start_repeat_" << this_many << ":;\n";
        }
        out << "prev_" << temp << "= " << temp << ";\n"
            << temp << " = (typeof(" << temp << "))n_malloc(tmparena,sizeof(*"
            << temp << "));\n"
        ...
        out << count << "= " << iexpr << ";\n";
        out << data << "= (typeof(" << data << "))n_malloc(arena,sizeof("
            << elem << ")*" << count << ");\n";
        out << "while(" << iter << "){" << iter << "--;\n"
            << "memcpy(&" << elem << ",&" << temp << "->elem,sizeof(" << elem
            << "));" << temp << " = (typeof(" << temp << "))" << temp
            << "->prev;\n"
            << "}";
        out << "}";
        break;
      ...
      case LENGTH: {
        ...
        out << data << " = (typeof(" << data << "))n_malloc(arena," << count
            << "*sizeof(" << elem << "));";
      ...
      case APPLY: {
        ...
        int this_many = num_iters++;
        out << "{/*APPLY*/";
        // XXX: These rewinds are not be necessary if name != current. Produce
        ...
        if (option::templates) {
          ...
          out << "NailStreamPos back_" << this_many
              << "= stream_getpos(str_current);";
          out << "str_current = " << stream_ptr << ";\n";
        }
        parser(p.apply.inner->pr, lval,
      ...
      case OPTIONAL: {
        ...
        out << "{/*Optional*/\n";
        if (option::templates)
          out << "typename strt_current::pos_t back_" << this_many
              << "= str_current->getpos();";
        else
          out << "NailStreamPos back_" << this_many
              << "= stream_getpos(str_current);";
        out << "NailArenaPos backarena_" << this_many
            << " = n_arena_save(arena);";
        parser(p.optional->pr, deref,
               (boost::format("goto fail_optional_%d;") % this_many).str(),
               scope);
      ...
      case REF: {
        ...
        out << lval << "= (typeof(" << lval << ")) n_malloc(" << arena
            << ",sizeof(*" << lval << "));\n";
        out << "if(parser_fail(peg_" << mk_str(p.name.name)
            << "(arena,tmparena, " << lval << ",str_current " << parameters
            << "))) { " << fail << "}\n";
        break;
      }
      ...
      if (def->N_type == ENDIAN) {
        ...
        if (option::templates) {
          ...
          forward_declarations
              << "template <" << templatepar << "> static int32_t peg_"
              << mk_str(def->parser.name)
              << "(NailArena *arena,NailArena *tmparena,"
              << mk_str(def->parser.name) << " *out,strt_current *str_current"
              << params << ");\n";
          ...
          forward_declarations
              << "static int32_t peg_" << mk_str(def->parser.name)
              << "(NailArena *arena,NailArena *tmparena,"
              << mk_str(def->parser.name) << " *out,NailStream *str_current"
              << params << ");\n";
          out << "static int32_t peg_" << name
              << "(NailArena *arena,NailArena *tmparena,"
              << mk_str(def->parser.name) << " *out,NailStream *str_current"
              << params << "){\n";
        }
        ...
        if (!def->parser.parameters || def->parser.parameters->count == 0) {
          ...
          // XXX: n_malloc
          if (option::templates)
            out << "template <typename stream_t> " << name << "* parse_" << name
                << "(NailArena *arena, stream_t *stream){\n";
          else
            out << name << "* parse_" << name
                << "(NailArena *arena, NailStream *stream){\n";
          out << name << "*retval = (" << name
              << "*)n_malloc(arena, sizeof(*retval));"
              << "NailArena tmparena;"
              << "NailArena_init(&tmparena, 4096, arena->error_ret);"
              << "if(parser_fail(peg_" << name
              << "(arena, &tmparena,retval, stream))){"
              << "goto fail;"
              << "}";
          if (option::templates)
        ...
        if (option::templates) {
          out << "template <typename strt_current> static  int32_t peg_" << name
              << "(strt_current *str_current){\n";
          forward_declarations
              << "template <typename strt_current> static int32_t peg_" << name
              << "(strt_current *str_current);";
        } else {
          out << "static int32_t peg_" << name
              << "(NailStream *str_current){\n";
          forward_declarations << "static int32_t peg_"
                               << mk_str(def->constantdef.name)
                               << "(NailStream *str_current);\n";
        }
  ...
  CDirectParser(std::ostream* os, std::ostream* hedr)
      : final(*os), hdr(*hedr), nr_dep(0), num_iters(1), End(Big) {}
};
void emit_directparser(std::ostream* out, std::ostream* header,
                       grammar* grammar) {
  CDirectParser p(out, header);
  std::string impl_template((char*)nail_parser_template_c,
                            nail_parser_template_c_len);
  std::string cpp_template((char*)nail_parser_template_cc,
