...
extern unsigned int nail_parser_template_c_len, nail_parser_template_cc_len,
...
#define FOREACH(val, coll)                            \
  for (__typeof__((coll).elem[0])* val = (coll).elem; \
       val < (coll).elem + (coll).count; val++)
extern void emit_generator(std::ostream* out, std::ostream* header,
                           grammar* grammar);
extern void emit_directparser(std::ostream* out, std::ostream* header,
                              grammar* grammar);
extern void emit_parser(std::ostream* out, std::ostream* header,
                        grammar* grammar);
...
struct Dependency {
  ...
  uint64_t width;  // XXX: Replace by a line that should be inserted to output
                   // this Dependency
};
class Scope {
