    ...
    switch (p.N_type) {
      ...
      case STRUCTURE:
        ...
        FOREACH(field, p.structure) {
          if (field->N_type != FIELD) continue;
      ...
      case NUNION: {
        ...
        FOREACH(elem, p.nunion) {
          ...
          // emit_type(tmp,**elem);
          if (idx++ == 0) emit_type(**elem);
          //            first = tmp.str();
          else {
            // TODO: Perform in-depth checking
            //         if(first!= tmp.str()){
            //        throw std::runtime_error(boost::str(boost::format("UNION
            //        elements do not induce the same type\n %s \n %s") %
            //        tmp.str() % first)); //Be more
            // descriptive
            //    }
          }
        }
      ...
      case REF:
        out << mk_str(p.ref.name)
            << "* ";  // TODO: Make ref more powerful, featuring ALIAS
        break;
      case NAME:
  ...
  void emit_parser(grammar* grammar) {
    try {
      ...
      FOREACH(definition, *grammar) {
        if (definition->N_type != PARSER) continue;
        std::string name(mk_str(definition->parser.name));
        std::string post;
        std::string typedef_t(
            typedef_type(definition->parser.definition, name, &post));
        out << "typedef " << typedef_t << " " << name << post << ";"
            << std::endl;
      }
      FOREACH(definition, *grammar) { emit_type(*definition); }
      out << std::endl;
...
void emit_header(std::ostream* out, grammar* grammar) {
  ...
  *out << std::string((char*)nail_parser_template_h,
                      nail_parser_template_h_len);
  FOREACH(def, *grammar) {
    if (def->N_type == PARSER) {
      std::string name = mk_str(def->parser.name);
      if (option::cpp)
        *out << name << "*parse_" << name
             << "(NailArena *arena, NailStream *data);\n";
      else
        *out << name << "*parse_" << name
