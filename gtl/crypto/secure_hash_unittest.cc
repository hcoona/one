...
TEST(SecureHashTest, TestUpdate) {
  ...
  std::unique_ptr<gtl::crypto::SecureHash> ctx(
      gtl::crypto::SecureHash::Create(gtl::crypto::SecureHash::SHA256));
...
TEST(SecureHashTest, TestClone) {
  ...
  const uint8_t kExpectedHashOfInput1[gtl::crypto::kSHA256Length] = {
  ...
  const uint8_t kExpectedHashOfInput1And2[gtl::crypto::kSHA256Length] = {
  ...
  std::unique_ptr<gtl::crypto::SecureHash> ctx1(
      gtl::crypto::SecureHash::Create(gtl::crypto::SecureHash::SHA256));
  ctx1->Update(input1.data(), input1.size());

  std::unique_ptr<gtl::crypto::SecureHash> ctx2(ctx1->Clone());
  std::unique_ptr<gtl::crypto::SecureHash> ctx3(ctx2->Clone());
  ...
  EXPECT_EQ(0, memcmp(output1, output2, gtl::crypto::kSHA256Length));
  EXPECT_EQ(0,
            memcmp(output1, kExpectedHashOfInput1And2, gtl::crypto::kSHA256Length));

  // Finish() ctx3, which should produce the hash of input1.
  ctx3->Finish(&output3, sizeof(output3));
  EXPECT_EQ(0, memcmp(output3, kExpectedHashOfInput1, gtl::crypto::kSHA256Length));
}

TEST(SecureHashTest, TestLength) {
  std::unique_ptr<gtl::crypto::SecureHash> ctx(
      gtl::crypto::SecureHash::Create(gtl::crypto::SecureHash::SHA256));
  EXPECT_EQ(gtl::crypto::kSHA256Length, ctx->GetHashLength());
}
...
TEST(SecureHashTest, Equality) {
  ...
  std::unique_ptr<gtl::crypto::SecureHash> ctx2(
      gtl::crypto::SecureHash::Create(gtl::crypto::SecureHash::SHA256));
