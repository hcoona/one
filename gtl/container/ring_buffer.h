...
#ifndef GTL_CONTAINER_RING_BUFFER_H_
#define GTL_CONTAINER_RING_BUFFER_H_
 ...
 public:
  ...
  class Iterator {
   public:
    ...
    Iterator& operator++() {
      index_++;
      if (index_ == kSize) out_of_range_ = true;
      return *this;
    }

    Iterator& operator--() {
      if (index_ == 0) out_of_range_ = true;
      index_--;
      return *this;
    }
  ...
  Iterator Begin() const {
    if (current_index_ < kSize) return Iterator(*this, kSize - current_index_);
...
#endif  // GTL_CONTAINER_RING_BUFFER_H_
