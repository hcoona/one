...
//
// Modified from
// https://github.com/tensorflow/tensorflow/blob/v2.8.0/tensorflow/core/platform/default/posix_file_system.cc
// https://github.com/tensorflow/tensorflow/blob/v2.8.0/tensorflow/core/platform/errors.cc
//
...
class PosixReadOnlyMemoryRegion : public ReadOnlyMemoryRegion {
 public:
  ...
  ~PosixReadOnlyMemoryRegion() override {
    ::munmap(const_cast<void*>(address_), length_);  // NOLINT
  }

  PosixReadOnlyMemoryRegion(const PosixReadOnlyMemoryRegion&) = delete;
  PosixReadOnlyMemoryRegion& operator=(const PosixReadOnlyMemoryRegion&) =
      delete;

  PosixReadOnlyMemoryRegion(PosixReadOnlyMemoryRegion&&) = default;
  PosixReadOnlyMemoryRegion& operator=(PosixReadOnlyMemoryRegion&&) = default;
...
absl::Status PosixFileSystem::NewReadOnlyMemoryRegionFromFile(
  ...
  if (fd < 0) {
    ...
    if (address == MAP_FAILED) {  // NOLINT
      s = IOError(fname, errno);
    } else {
      *result =
          std::make_unique<PosixReadOnlyMemoryRegion>(address, st.st_size);
    }
    if (close(fd) < 0) {
      s = IOError(fname, errno);
    }
