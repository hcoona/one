...
//
// Copied from https://github.com/microsoft/GSL/blob/v4.0.0/include/gsl/span
// Modifications:
// 1. Use #pragma once header guard.
// 2. Re-format with Google style.
// 3. Remove GSL dependencies.
// 4. Rename namespace `gsl` to `hcoona`.
// 5. Rename macro prefix `ONE_` to `ONE_`.

#pragma once

#include <array>        // for array
#include <cstddef>      // for byte, ptrdiff_t, size_t, nullptr_t
...
// turn off some warnings that are noisy about our ONE_HARDENING_ASSERT
// statements
...
#pragma warning(disable : 26495)  // uninitalized member when constructor calls
                                  // constructor
#pragma warning(disable : 26446)  // parser bug does not allow attributes on
                                  // some templates
...
#define ONE_USE_STATIC_CONSTEXPR_WORKAROUND
...
class span_iterator {
 public:
  ...
  constexpr reference operator*() const noexcept {
    ONE_HARDENING_ASSERT(begin_ && end_);
    ONE_HARDENING_ASSERT(begin_ <= current_ && current_ < end_);
    return *current_;
  }
  ...
  constexpr span_iterator& operator++() noexcept {
    ONE_HARDENING_ASSERT(begin_ && current_ && end_);
  ...
  constexpr span_iterator& operator--() noexcept {
    ONE_HARDENING_ASSERT(begin_ && end_);
    ONE_HARDENING_ASSERT(begin_ < current_);
    --current_;
    return *this;
  }
  ...
  constexpr span_iterator& operator+=(const difference_type n) noexcept {
    if (n != 0) ONE_HARDENING_ASSERT(begin_ && current_ && end_);
    if (n > 0) ONE_HARDENING_ASSERT(end_ - current_ >= n);
    if (n < 0) ONE_HARDENING_ASSERT(current_ - begin_ >= -n);
    current_ += n;
    return *this;
  }
  ...
  constexpr difference_type operator-(
    ...
    ONE_HARDENING_ASSERT(begin_ == rhs.begin_ && end_ == rhs.end_);
  ...
  friend constexpr void _Verify_range(
    ...
    ONE_HARDENING_ASSERT(lhs.begin_ == rhs.begin_  // range spans have to match
                         && lhs.end_ == rhs.end_ &&
                         lhs.current_ <=
                             rhs.current_);  // range must not be transposed
  }
  ...
  constexpr pointer _Unwrapped()
      const noexcept {  // after seeking *this to a high water mark, or using
                        // one of the
...
#if defined(ONE_USE_STATIC_CONSTEXPR_WORKAROUND)
  static constexpr const bool _Unwrap_when_unverified = false;
#else
  static constexpr bool _Unwrap_when_unverified = false;
#endif
  constexpr void _Seek_to(
      const pointer p) noexcept {  // adjust the position of *this to previously
                                   // verified location p
    // after _Unwrapped
    current_ = p;
  }
...
class extent_type {
 public:
  ...
  constexpr explicit extent_type(size_type size) {
    ONE_HARDENING_ASSERT(size == Ext);
  }
...
class extent_type<dynamic_extent> {
 public:
  ...
  constexpr explicit extent_type(size_type size) : size_(size) {
    ONE_HARDENING_ASSERT(size != dynamic_extent);
  }
...
constexpr extent_type<Ext>::extent_type(extent_type<dynamic_extent> ext) {
  ONE_HARDENING_ASSERT(ext.size() == Ext);
}
...
class span {
 public:
  ...
  constexpr explicit span(pointer ptr, size_type count) noexcept
      : storage_(ptr, count) {
    ONE_HARDENING_ASSERT(count == Extent);
  }
  ...
  constexpr explicit span(pointer firstElem, pointer lastElem) noexcept
    ...
    ONE_HARDENING_ASSERT(lastElem - firstElem ==
                         static_cast<difference_type>(Extent));
  }
  ...
  constexpr span<element_type, Count> first() const noexcept {
    ONE_HARDENING_ASSERT(Count <= size());
  ...
  constexpr auto subspan() const noexcept ->
      typename details::calculate_subspan_type<ElementType, Extent, Offset,
                                               Count>::type {
    ONE_HARDENING_ASSERT((size() >= Offset) && (Count == dynamic_extent ||
                                                (Count <= size() - Offset)));
  ...
  constexpr size_type size_bytes() const noexcept {
    ONE_HARDENING_ASSERT(size() < dynamic_extent / sizeof(element_type));
  ...
  constexpr reference operator[](size_type idx) const noexcept {
    ONE_HARDENING_ASSERT(idx < size());
    return data()[idx];
  }

  constexpr reference front() const noexcept {
    ONE_HARDENING_ASSERT(size() > 0);
    return data()[0];
  }
 ...
 private:
  ...
  class storage_type : public ExtentType {
   public:
    ...
    constexpr storage_type(KnownNotNull data, OtherExtentType ext)
      ...
      ONE_HARDENING_ASSERT(ExtentType::size() != dynamic_extent);
    }
    ...
    constexpr storage_type(pointer data, OtherExtentType ext)
      ...
      ONE_HARDENING_ASSERT(ExtentType::size() != dynamic_extent);
      ONE_HARDENING_ASSERT(data || ExtentType::size() == 0);
    }
  ...
  template <std::size_t CallerExtent>
  constexpr span<element_type, dynamic_extent> make_subspan(
  ...
  constexpr span<element_type, dynamic_extent> make_subspan(
    ...
    ONE_HARDENING_ASSERT(size() >= offset);

    if (count == dynamic_extent) {
      return {KnownNotNull{data() + offset}, size() - offset};
    }

    ONE_HARDENING_ASSERT(size() - offset >= count);
...
template <class Container,
          class Element = std::remove_pointer_t<
              decltype(std::declval<const Container&>().data())>>
...
as_bytes(span<ElementType, Extent> s) noexcept {
  using type = span<const std::byte,
